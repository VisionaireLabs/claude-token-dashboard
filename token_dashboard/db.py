...
def _migrate_add_source(conn) -> None:
    """Add messages.source and tool_calls.source for Cowork support.

    Pre-migration rows are backfilled to 'claude_code' since the upstream
    project only scanned ~/.claude/projects/. New scans tag rows by source.
    """
    ...
    if "source" not in msg_cols:
        conn.execute("ALTER TABLE messages ADD COLUMN source TEXT NOT NULL DEFAULT 'claude_code'")
    tool_cols = {row[1] for row in conn.execute("PRAGMA table_info(tool_calls)")}
    if "source" not in tool_cols:
        conn.execute("ALTER TABLE tool_calls ADD COLUMN source TEXT NOT NULL DEFAULT 'claude_code'")
    conn.commit()
...
def _source_clause(source, col: str = "source"):
    """Return a SQL fragment + bound args for the source filter.

    source values:
      None or "all" / "" → no filter (returns empty string)
      "claude_code" / "cowork" → filter to that single source
    """
    if not source or source == "all":
        return "", []
    if source not in ("claude_code", "cowork"):
        return "", []
...
def best_project_name(cwds, slug: str) -> str:
    ...
    Cowork slugs (`cowork:<workspace-uuid>`) bypass the cwd walk entirely and
    are formatted directly — Cowork sessions don't have a stable on-disk
    project root the way Claude Code projects do, so the walk would never
    match and the fallback would split the UUID on "-" and pick the last
    segment, which is meaningless.
    """
    if slug and slug.startswith("cowork:"):
        ws = slug.split(":", 1)[1] if ":" in slug else slug
        # Show first 8 chars of the workspace UUID, like a short git SHA.
        short = ws.split("-", 1)[0][:8] if ws else "workspace"
        return f"Cowork · {short}"
...
def overview_totals(db_path, since=None, until=None, source=None) -> dict:
    rng, rng_args = _range_clause(since, until)
    ...
    with connect(db_path) as c:
        return dict(c.execute(sql, rng_args + src_args).fetchone())
...
def expensive_prompts(db_path, limit: int = 50, sort: str = "tokens", source=None) -> list:
    ...
    src_u, src_args = _source_clause(source, col="u.source")
    sql = f"""
      SELECT u.uuid AS user_uuid, u.session_id, u.project_slug, u.timestamp,
       ...
       WHERE u.type='user' AND u.prompt_text IS NOT NULL{src_u}
       ORDER BY {order}
       LIMIT ?
    """
    with connect(db_path) as c:
...
def project_summary(db_path, since=None, until=None, source=None) -> list:
    ...
    src_q, src_args = _source_clause(source, col="m.source")
    sql = f"""
      SELECT project_slug,
...
def tool_token_breakdown(db_path, since=None, until=None, source=None) -> list:
    rng, args = _range_clause(since, until)
    src_q, src_args = _source_clause(source)
    sql = f"""
      SELECT tool_name,
             COUNT(*) AS calls,
             COALESCE(SUM(result_tokens),0) AS result_tokens
        FROM tool_calls
       WHERE tool_name != '_tool_result' {rng}{src_q}
       GROUP BY tool_name
       ORDER BY calls DESC
    """
    with connect(db_path) as c:
...
def daily_token_breakdown(db_path, since=None, until=None, source=None) -> list:
    ...
    sql = f"""
      SELECT substr(timestamp, 1, 10) AS day,
        ...
        FROM messages
       WHERE timestamp IS NOT NULL {rng}{src_q}
       GROUP BY day
       ORDER BY day ASC
    """
    with connect(db_path) as c:
...
def skill_breakdown(db_path, since=None, until=None, source=None) -> list:
    ...
    sql = f"""
      SELECT target AS skill,
        ...
        FROM tool_calls
       WHERE tool_name = 'Skill' AND target IS NOT NULL AND target != '' {rng}{src_q}
       GROUP BY target
...
def model_breakdown(db_path, since=None, until=None, source=None) -> list:
    ...
    sql = f"""
      SELECT COALESCE(model, 'unknown') AS model,
             ...
             COALESCE(SUM(cache_create_1h_tokens),0)  AS cache_create_1h_tokens
        FROM messages
       WHERE type = 'assistant' {rng}{src_q}
       GROUP BY model
