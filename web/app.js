...
function buildTopbar() {
  ...
  wrap.innerHTML = `
    <div class="brand"><span class="name">Token Dashboard</span><span class="org">Visionaire Labs</span></div>
    <nav>
    ...
    <div class="source-toggle" id="source-toggle">
      ...
      <button data-src="claude_code">code</button>
      <button data-src="cowork">cowork</button>
    </div>
...
async function boot() {
  ...
  // Source filter: persisted in localStorage, applied via query param ?source=
  const savedSource = localStorage.getItem('td.source') || 'all';
  state.source = savedSource;
  $$('#source-toggle button').forEach(b => {
    b.classList.toggle('active', b.dataset.src === savedSource);
    b.addEventListener('click', () => {
      state.source = b.dataset.src;
      localStorage.setItem('td.source', state.source);
      $$('#source-toggle button').forEach(x => x.classList.toggle('active', x === b));
      render();
    });
  });
