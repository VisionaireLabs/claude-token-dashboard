...
export default async function (root) {
  ...
  stackedBarChart(document.getElementById('ch-daily-billable'), {
    ...
    series: [
      { name: 'input',        values: daily.map(d => d.input_tokens) },
      { name: 'output',       values: daily.map(d => d.output_tokens) },
      { name: 'cache create', values: daily.map(d => d.cache_create_tokens) },
    ],
  });
  ...
  stackedBarChart(document.getElementById('ch-daily-cache'), {
    categories: daily.map(d => d.day),
    series: [
      { name: 'cache read', values: daily.map(d => d.cache_read_tokens) },
    ],
  });
  ...
  groupedBarChart(document.getElementById('ch-projects'), {
    ...
    series: [
      { name: 'input',  values: topProjects.map(p => p.input_tokens  || 0) },
      { name: 'output', values: topProjects.map(p => p.output_tokens || 0) },
    ],
  });
